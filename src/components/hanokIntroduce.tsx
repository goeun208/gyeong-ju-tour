...
const HanokIntroduce = ({element3}: any) => {
    ...
    const hanok = [hanok1, hanok2, hanok3, hanok4, hanok5, hanok6, hanok7];
    ...
    // style={{ background: 'linear-gradient(180deg, rgba(8,17,19,1) 0%, rgba(95,102,104,1) 49%, rgba(143,151,153,1) 100%)'}}
    return (
         data && 
         <div ref={element3}  className="bg-zinc-50 py-20 relative overflow-hidden" >
           <img src={back2} alt="banner" className="absolute top-0 left-0 w-full h-full object-cover object-center opacity-20" loading="lazy" decoding="async"/>
             ...
             {data.map((item:any, index:number) => (
               <div key={index} className='w-[21.5rem] md:w-[29.5rem] h-[25.5rem] bg-zinc-200 text-[1.125rem] text-black ml-4 md:mx-2 rounded cursor-pointer'>
                 <div className=' w-[21.5rem] md:w-[29.5rem] h-[25.5rem] mx-auto rounded relative'>
                   ...
                   { item.HMPG_NM && <button className="bg-[rgba(255,255,255,0.4)] px-3 py-1 rounded-lg absolute top-5 right-5 text-[0.9rem]"><a href={item.HMPG_NM}>상세보기</a></button> }
